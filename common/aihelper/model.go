...
func (o *AliRAGModel) GenerateResponse(ctx context.Context, messages []*schema.Message) (*schema.Message, error) {
	...
	if err != nil {
		...
		return schema.AssistantMessage("您还没有上传知识库文档。请先上传 .md 或 .txt 文件，并选择「RAG 知识库」模式。", nil), nil
	}

	docs, err := ragQuery.RetrieveDocuments(ctx, query)
	if err != nil {
		log.Printf("Failed to retrieve documents: %v", err)
		return schema.AssistantMessage("检索知识库失败，请确认 Redis Stack 已启动且文档已成功上传。", nil), nil
	}
...
func (o *AliRAGModel) StreamResponse(ctx context.Context, messages []*schema.Message, cb StreamCallback) (string, error) {
	...
	if err != nil {
		log.Printf("Failed to create RAG query: %v", err)
		return o.streamFixedMessage("您还没有上传知识库文档。请先点击右上角 📎 上传 .md 或 .txt 文件，并选择「RAG 知识库」模式后再提问。", cb)
	}

	docs, err := ragQuery.RetrieveDocuments(ctx, query)
	if err != nil {
		log.Printf("Failed to retrieve documents: %v", err)
		return o.streamFixedMessage("检索知识库失败，请确认 Redis Stack 已启动且文档已成功上传。", cb)
	}

	if len(docs) == 0 {
		log.Printf("RAG retrieve returned 0 docs for user %s", o.username)
	}
...
// streamFixedMessage 返回固定提示（如无文档、检索失败）
func (o *AliRAGModel) streamFixedMessage(text string, cb StreamCallback) (string, error) {
	cb(text)
	return text, nil
}
