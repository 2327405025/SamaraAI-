...
const (
	...
	UserNameMsg = "SamaraAI的账号如下，请保留好，后续可以用账号/邮箱登录 "
)
...
func SendCaptcha(email, code, msg string) error {
	...
	// 发件人
	m.SetHeader("From", config.Get().Email.Email)
	// 收件人
	m.SetHeader("To", email)
	// 主题
	m.SetHeader("Subject", "来自SamaraAI的信息")
	// 正文内容（纯文本形式，也可以用 text/html）
	m.SetBody("text/plain", msg+" "+code)

	// 配置 SMTP 服务器和授权码,587：是 SMTP 的明文/STARTTLS 端口号
	d := gomail.NewDialer("smtp.qq.com", 587, config.Get().Email.Email, config.Get().Email.Authcode)
